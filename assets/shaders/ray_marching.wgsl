...
fn new_hyperplane(origin: vec4<f32>, n: vec4<f32>) -> Hyperplane {
  ...
  let v_direction =
      normalize(basis[1] - normal * dot(normal, basis[1]) - u_direction * dot(u_direction, basis[1]));
  let w_direction = normalize(basis[2]
    - normal * dot(normal, basis[2])
    - u_direction * dot(u_direction, basis[2])
    - v_direction * dot(v_direction, basis[2]));
